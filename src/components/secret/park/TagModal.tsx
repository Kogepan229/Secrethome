...
const TagModal = (props: Props) => {
  ...
  const TagItem = (tagProps: TagProps) => {
    ...
    return <div className={css.tag} onClick={onClickTagItem}>{tagProps.tag.name}</div>
  }

  const TagItems = props.tagList.filter(value => {
  ...
  const createTag = () => {
    ...
    if (createTagValue.trim()) {
      if (!props.tagList.some(value => { return value.name === createTagValue.trim() })) {
        console.log("ok")
    ...
    <div className={css.modal_container}>
      ...
      <div className={css.tags_container}>
        <div className={css.create_tag}>
          <input className={css.create_tag_input} spellCheck="false" type="text" value={createTagValue} onChange={e => setCreatetagValue(e.target.value)}></input>
          <button className={css.create_tag_button} onClick={createTag} disabled={!createTagValue}>追加</button>
        </div>
        {TagItems}
