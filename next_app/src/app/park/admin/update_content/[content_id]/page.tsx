...
const getContentData = async (contentID: any) :Promise<ContentData> => {
  ...
  if (result.length == 0) {
    ...
    return { id: contentID as (string | undefined), title: result[0].title, description: result[0].description, tags: JSON.parse(JSON.stringify(resultTag)), selectedTags: await getContentTagsData(contentID as string)}
  }
}
...
const UpdateContent = async ({ params }: {params: any}) => {
  ...
  return (
      <div>
        <EditContentForm isUpdate={true} id={contentData.id} title={contentData.title} description={contentData.description} tags={contentData.tags} selectedTags={contentData.selectedTags}></EditContentForm>
