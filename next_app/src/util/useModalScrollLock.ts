// 参照
// https://zenn.dev/k_kazukiiiiii/articles/612048066a78ae
...
export const useModalScrollLock = (isModalOpen: boolean, divID: string) => {

  /** 指定した要素以外のスクロールを抑止する関数（iOS Safariの場合のみの制御） */
  const scrollLock = (event: TouchEvent) => {
    const canScrollElement = (event.target as HTMLDivElement)?.closest(`#${divID}`);
  ...
  const scrollLockFixAdd = (element: HTMLElement) => {
    const canScrollElement = element.querySelector<HTMLDivElement>(`#${divID}`);
  ...
  const scrollStopBackContent = () => {
    const canScrollModalElement = document.querySelector<HTMLDivElement>(`#${divID}`);
