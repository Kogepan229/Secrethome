...
const ContentPost = (props: Props) => {
  ...
  const Tags = props.tags.map(value => {
    ...
    <Link href={`/park/tags/${value.id}`} key={value.id}>
      <SimpleButton className={css.content_tag} onClick={(e) => e.stopPropagation()}>{value.name}</SimpleButton>
    </Link>
    )
  })
    ...
    <div className={css.content_post} onClick={onClickPost}>
      <div className={css.content_post_inside}>
        <Link href={`/park/contents/${props.id}`}>
          <img className={css.content_img} src={`${process.env.NEXT_PUBLIC_FILESERVER_URL}/contents/${props.id}/${props.id}.webp?${props.updated_at}`} onError={onErrorImage} onClick={(e) => e.stopPropagation()}></img>
        </Link>
